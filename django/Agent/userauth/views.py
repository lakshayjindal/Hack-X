...
def index(request):
    json_data = './userauth/credentials.json'
    validator = auth(json_data)
    print(validator)
...
def authorize(api_url,client_id,client_secret,redirect_uri):
    ...
    redirect_response = input('https://www.linkedin.com/oauth/v2/authorization?response_type=code&client_id=86xjct2jz6yt5u&redirect_uri=http%3A%2F%2Flocalhost%3A8000&state=hdorpdpvypbwbknegdwn&scope=r_liteprofile%2Cr_emailaddress%2Cw_member_social')
